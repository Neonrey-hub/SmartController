...
**更新** 新增自动重连逻辑、WiFi状态监控、设备识别和会话管理功能
...
- **网络稳定性**
  ...
  - 自动重连：使用防抖机制避免频繁重连，延迟1.5秒等待网络稳定。
- **音频传输**
...
**更新** 反映WebSocket连接管理系统的重要增强

本项目在设备侧与语音识别两条 WebSocket 路径上提供了完整的生命周期管理与事件处理机制。设备侧连接具备断线检测与自动重连能力，包括：
- WiFi状态监控与自动重连
- 设备识别（MAC地址和UUID）
- 会话管理（session_id）
- 请求队列管理
...
语音识别连接具备鉴权、音频帧发送与结果解析能力。建议在多并发场景下引入连接池与队列调度，并完善二进制消息处理与缓存清理策略，以进一步提升稳定性与性能。
