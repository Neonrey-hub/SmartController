...
控制按钮组件是SmartController项目中设备联动控制台的核心交互组件之一。该组件实现了三种控制模式的切换功能，包括展示模式、告警模式和静音模式。通过单选机制确保同一时间只有一个按钮处于高亮状态，为用户提供清晰的视觉反馈和直观的操作体验。
...
CreateRow --> AddButtons[添加三个ControlButton<br/>展示模式/告警模式/静音模式]
