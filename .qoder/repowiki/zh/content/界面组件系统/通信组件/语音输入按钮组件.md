...
- [ChatPage.ets:1-102](file://entry/src/main/ets/pages/ChatPage.ets#L1-L102)
...
**更新** 网络连接导入路径已从 `'ets/pages/network_connect'` 更正为 `'ets/network/network_connect'`，以匹配新的模块结构。
...
**更新** 网络连接文件已从 `ets/pages/network_connect.ets` 移动到 `ets/network/network_connect.ets`，对应的类名从 `network_connect` 更改为 `WebSocket_connect`，并通过导出实例 `network_connect` 提供访问。
...
**更新** 导入路径已调整以反映新的模块结构，从 `ets/pages/network_connect` 更正为 `ets/network/network_connect`。
...
**更新** 网络连接导入路径错误会导致模块找不到，需确保导入路径与实际文件位置一致。
...
- [network_connect.ets:253-261](file://entry/src/main/ets/network/network_connect.ets#L253-L261)
...
**更新** 新的模块结构改进了代码组织，将网络连接功能移动到 `ets/network/` 目录下，使项目结构更加清晰和一致。导入路径的修正确保了模块间的正确依赖关系。